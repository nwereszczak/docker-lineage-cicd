...
RUN cd /root/ && \
        ...
        rm -rf OpenDelta/ && \
        OUT_DIR=$(echo '$BRANCH_NAME' | sed 's/[^[:alnum:]]/_/g') && \
        OUT_DIR=$SRC_DIR/${OUT_DIR^^}
        sed -i -e 's|^\s*HOME=.*|HOME=/root|; \
                   s|^\s*BIN_XDELTA=.*|BIN_XDELTA=xdelta3|; \
                   s|^\s*FILE_MATCH=.*|FILE_MATCH=lineage-\*.zip|; \
                   s|^\s*PATH_CURRENT=.*|PATH_CURRENT=$OUT_DIR/out/target/product/$DEVICE|; \
                   s|^\s*PATH_LAST=.*|PATH_LAST=$OUT_DIR/delta_last/$DEVICE|; \
